...
import SystemDiagramConfig from './system-diagram-config';
...
export class SystemDiagramComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public systemDiagramConfig = input.required<SystemDiagramConfig>();
  ...
  private buildInfrastructure() {
    ...
    this.systemDiagramConfig().nodes.forEach((node) => {
    ...
    this.systemDiagramConfig().connections.forEach((conn) => {
  ...
  private animate = () => {
    ...
    this.nodeMeshes.forEach((mesh, id) => {
      ...
      mesh.position.y =
        this.systemDiagramConfig().nodes.find((n) => n.id === id)!.position.y +
